...
EndProcedure
   ...
   ; Helper function to determine result type of an expression
   Procedure.w          GetExpressionType( *x.stTree )
      Protected         n, leftType.w, rightType.w
      ...
      Select *x\NodeType
         ...
         Case #ljAdd
            ; ADD with any string operand produces string
            leftType = GetExpressionType(*x\left)
            rightType = GetExpressionType(*x\right)
            ...
            Else
               ProcedureReturn #C2FLAG_INT
            EndIf
         Case #ljSUBTRACT, #ljMULTIPLY, #ljDIVIDE, #ljMOD
            ; Arithmetic operations: float if either operand is float
            leftType = GetExpressionType(*x\left)
            rightType = GetExpressionType(*x\right)
            ...
            EndIf
         Case #ljNEGATE
            ProcedureReturn GetExpressionType(*x\left)
         Default
   ...
   Procedure            CodeGenerator( *x.stTree, *link.stTree = 0 )
      ...
      Select *x\NodeType
         ...
         Case #ljLESS, #ljGREATER, #ljLESSEQUAL, #ljGreaterEqual, #ljEQUAL, #ljNotEqual,
            ...
            ; Determine operand types using helper function
            Protected leftType.w = GetExpressionType(*x\left)
            Protected rightType.w = GetExpressionType(*x\right)

            ; Special handling for string concatenation (ADD operation only)
            If *x\NodeType = #ljAdd And (leftType = #C2FLAG_STR Or rightType = #C2FLAG_STR)
               ; Generate left operand
               CodeGenerator( *x\left )

               ; Convert left to string if needed
               If leftType <> #C2FLAG_STR
               ...
               EmitInt( #ljSTRADD )
            Else
               ; Standard arithmetic/comparison operations
               CodeGenerator( *x\left )
               CodeGenerator( *x\right )

               ; Determine operation type (prioritize float over int)
               opType.w = #C2FLAG_INT
