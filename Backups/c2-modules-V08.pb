      ...
      Select TOKEN()\TokenType
         ...
         Case #ljPrint
            ...
            Repeat
               ...
               ElseIf TOKEN()\TokenExtra = #ljFLOAT
                  *r = expr( 0 )
                  printType = #ljPRTF
               ElseIf TOKEN()\TokenExtra = #ljIDENT
                  ; Check variable type
                  varIdx = FetchVarOffset(TOKEN()\value)
                  
                  If gVar(varIdx)\flags & #C2FLAG_FLOAT
                     printType = #ljPRTF
                  ElseIf gVar(varIdx)\flags & #C2FLAG_STR
                     printType = #ljPRTS
                  EndIf
                  *r = expr( 0 )
               Else
                  *r = expr( 0 )
                  ; Could check expression result type here for complex expressions
               EndIf
         ...
         Case #ljIDENT
            *v = Makeleaf( #ljIDENT, TOKEN()\value )
            ; *** FIX #1: Copy type hint from token to tree node (for .f and .s suffixes) ***
      ...
      Else
         ...
         ElseIf TOKEN()\TokenType = #ljIDENT
            ...
            Else
               ...
               inferredType = 0
               If *assignmentTree
                  ; Look at what's being assigned
                  If *assignmentTree\NodeType = #ljINT
                     inferredType = #C2FLAG_INT
                  ElseIf *assignmentTree\NodeType = #ljFLOAT
                     inferredType = #C2FLAG_FLOAT
                  ElseIf *assignmentTree\NodeType = #ljSTRING
                     inferredType = #C2FLAG_STR
                  ; *** FIX #2: Type inference for arithmetic operations that produce floats or strings ***
                  ElseIf *assignmentTree\NodeType = #ljADD Or *assignmentTree\NodeType = #ljSUBTRACT Or
                         *assignmentTree\NodeType = #ljMULTIPLY Or *assignmentTree\NodeType = #ljDIVIDE
                     ; Check if left or right operand is a float or string
                     Protected.i leftIdx, rightIdx, checkIdx
                     leftIdx = -1
                     rightIdx = -1

                     If *assignmentTree\left
                        If *assignmentTree\left\NodeType = #ljFLOAT
                           inferredType = #C2FLAG_FLOAT
                        ElseIf *assignmentTree\left\NodeType = #ljSTRING
                           inferredType = #C2FLAG_STR
                        ElseIf *assignmentTree\left\NodeType = #ljIDENT
                           For checkIdx = 0 To gnLastVariable - 1
                              If gVar(checkIdx)\name = *assignmentTree\left\value
                                 leftIdx = checkIdx
                                 Break
                              EndIf
                           Next
                           If leftIdx >= 0
                              If gVar(leftIdx)\flags & #C2FLAG_FLOAT
                                 inferredType = #C2FLAG_FLOAT
                              ElseIf gVar(leftIdx)\flags & #C2FLAG_STR
                                 inferredType = #C2FLAG_STR
                              EndIf
                           EndIf
                        EndIf
                     EndIf
                     If inferredType = 0 And *assignmentTree\right
                        If *assignmentTree\right\NodeType = #ljFLOAT
                           inferredType = #C2FLAG_FLOAT
                        ElseIf *assignmentTree\right\NodeType = #ljSTRING
                           inferredType = #C2FLAG_STR
                        ElseIf *assignmentTree\right\NodeType = #ljIDENT
                           For checkIdx = 0 To gnLastVariable - 1
                              If gVar(checkIdx)\name = *assignmentTree\right\value
                                 rightIdx = checkIdx
                                 Break
                              EndIf
                           Next
                           If rightIdx >= 0
                              If gVar(rightIdx)\flags & #C2FLAG_FLOAT
                                 inferredType = #C2FLAG_FLOAT
                              ElseIf gVar(rightIdx)\flags & #C2FLAG_STR
                                 inferredType = #C2FLAG_STR
                              EndIf
                           EndIf
                        EndIf
                     EndIf
                  ; Check if this is a function call assignment (SEQ node with CALL)
                  ElseIf *assignmentTree\NodeType = #ljSEQ
                     ; Function call: SEQ(params, CALL)
                     ; The CALL node is in the right child
                     If *assignmentTree\right And *assignmentTree\right\NodeType = #ljCALL
                        ; Look up function's return type
                        Protected funcIndex.i = Val(*assignmentTree\right\value)
                        ; Find the function in mapModules by function index
                        ForEach mapModules()
                           If mapModules()\function = funcIndex
                              inferredType = mapModules()\returnType
                              Break
                           EndIf
                        Next
                     EndIf
   ...
   Procedure            CodeGenerator( *x.stTree, *link.stTree = 0 )
      ...
      Select *x\NodeType
         ...
         Case #ljPOP
            ...
            ; Use typeHint to determine variable type and emit correct POP opcode
         ...
         Case #ljASSIGN
            ; NEW: Pass assignment tree for type inference
            n = FetchVarOffset( *x\left\value, *x\right )
            ; *** FIX #3: Override inferred type if tree node has explicit type hint from suffix ***
         ...
         Case #ljLESS, #ljGREATER, #ljLESSEQUAL, #ljGreaterEqual, #ljEQUAL, #ljNotEqual,
            ...
            ; NEW: Emit typed opcode based on operand types
            ; Determine type from left operand (could be enhanced to check both)
            opType.w = #C2FLAG_INT
            Protected.w leftType, rightType

            If *x\left\NodeType = #ljIDENT
               n = FetchVarOffset(*x\left\value)
               opType = gVar(n)\flags & #C2FLAG_TYPE
               leftType = opType
            ElseIf *x\left\NodeType = #ljFLOAT
               opType = #C2FLAG_FLOAT
               leftType = #C2FLAG_FLOAT
            ElseIf *x\left\NodeType = #ljSTRING
               opType = #C2FLAG_STR
               leftType = #C2FLAG_STR
            ElseIf *x\left\NodeType = #ljAdd Or *x\left\NodeType = #ljSUBTRACT Or *x\left\NodeType = #ljMULTIPLY Or *x\left\NodeType = #ljDIVIDE
               ; *** FIX #7: Handle nested ADD expressions ***
               ; For nested expressions, recurse to find the leftmost leaf to determine result type
               Protected *tempLeft.stTree = *x\left
               While *tempLeft\NodeType = #ljAdd Or *tempLeft\NodeType = #ljSUBTRACT Or *tempLeft\NodeType = #ljMULTIPLY Or *tempLeft\NodeType = #ljDIVIDE
                  *tempLeft = *tempLeft\left
               Wend

               ; Check what type the leftmost operand is
               If *tempLeft\NodeType = #ljSTRING
                  leftType = #C2FLAG_STR
                  opType = #C2FLAG_STR
               ElseIf *tempLeft\NodeType = #ljFLOAT
                  leftType = #C2FLAG_FLOAT
                  opType = #C2FLAG_FLOAT
               ElseIf *tempLeft\NodeType = #ljIDENT
                  n = FetchVarOffset(*tempLeft\value)
            ...
            ; *** FIX #4: Type priority - STRING > FLOAT > INT ***
            ; Also check right operand
            ; Priority: STRING > FLOAT > INT
            ; If either operand is string, use string operation
            ; Else if either operand is float, use float operation
            If Not (opType & #C2FLAG_STR)  ; Don't override string type - check if STR bit is set
               If *x\right\NodeType = #ljIDENT
                  n = FetchVarOffset(*x\right\value)
                  rightType = gVar(n)\flags & #C2FLAG_TYPE
                  If gVar(n)\flags & #C2FLAG_STR
                     opType = #C2FLAG_STR  ; String takes priority
                  ElseIf gVar(n)\flags & #C2FLAG_FLOAT And Not (opType & #C2FLAG_FLOAT)
                     opType = #C2FLAG_FLOAT
                  EndIf
               ElseIf *x\right\NodeType = #ljSTRING
                  opType = #C2FLAG_STR  ; String takes priority
                  rightType = #C2FLAG_STR
               ElseIf *x\right\NodeType = #ljFLOAT
                  rightType = #C2FLAG_FLOAT
                  If Not (opType & #C2FLAG_FLOAT)
                     opType = #C2FLAG_FLOAT
                  EndIf
               ElseIf *x\right\NodeType = #ljAdd Or *x\right\NodeType = #ljSUBTRACT Or *x\right\NodeType = #ljMULTIPLY Or *x\right\NodeType = #ljDIVIDE
                  ; *** FIX #7: Handle nested expressions on right side ***
                  Protected *tempRight.stTree = *x\right
                  While *tempRight\NodeType = #ljAdd Or *tempRight\NodeType = #ljSUBTRACT Or *tempRight\NodeType = #ljMULTIPLY Or *tempRight\NodeType = #ljDIVIDE
                     *tempRight = *tempRight\left
                  Wend

                  If *tempRight\NodeType = #ljSTRING
                     rightType = #C2FLAG_STR
                     opType = #C2FLAG_STR
                  ElseIf *tempRight\NodeType = #ljFLOAT
                     rightType = #C2FLAG_FLOAT
                     If Not (opType & #C2FLAG_FLOAT)
                        opType = #C2FLAG_FLOAT
                     EndIf
                  ElseIf *tempRight\NodeType = #ljIDENT
                     n = FetchVarOffset(*tempRight\value)
                     If gVar(n)\flags & #C2FLAG_STR
                        rightType = #C2FLAG_STR
                        opType = #C2FLAG_STR
                     ElseIf gVar(n)\flags & #C2FLAG_FLOAT
               ...
               EndIf
            Else
               ; opType is already STRING, determine right type
               If *x\right\NodeType = #ljIDENT
                  n = FetchVarOffset(*x\right\value)
                  rightType = gVar(n)\flags & #C2FLAG_TYPE
               ElseIf *x\right\NodeType = #ljSTRING
               ...
               ElseIf *x\right\NodeType = #ljFLOAT
                  rightType = #C2FLAG_FLOAT
               ElseIf *x\right\NodeType = #ljAdd Or *x\right\NodeType = #ljSUBTRACT Or *x\right\NodeType = #ljMULTIPLY Or *x\right\NodeType = #ljDIVIDE
                  ; *** FIX #7: Handle nested expressions when opType is already STRING ***
                  Protected *tempRight2.stTree = *x\right
                  While *tempRight2\NodeType = #ljAdd Or *tempRight2\NodeType = #ljSUBTRACT Or *tempRight2\NodeType = #ljMULTIPLY Or *tempRight2\NodeType = #ljDIVIDE
                     *tempRight2 = *tempRight2\left
                  Wend

                  If *tempRight2\NodeType = #ljSTRING
                     rightType = #C2FLAG_STR
                  ElseIf *tempRight2\NodeType = #ljFLOAT
                     rightType = #C2FLAG_FLOAT
                  ElseIf *tempRight2\NodeType = #ljIDENT
                     n = FetchVarOffset(*tempRight2\value)
                     rightType = gVar(n)\flags & #C2FLAG_TYPE
                  Else
                     rightType = #C2FLAG_INT
                  EndIf
            ...
            ; *** FIX #6: Insert type conversion for string concatenation ***
            ; If operation is string concat and left operand is not string, convert it
            If opType & #C2FLAG_STR And (leftType & #C2FLAG_STR) = 0
            ...
            ; If operation is string concat and right operand is not string, convert it
            If opType & #C2FLAG_STR And (rightType & #C2FLAG_STR) = 0
            ...
            ElseIf opType & #C2FLAG_STR And gszATR(*x\NodeType)\strtoken > 0
               EmitInt( gszATR(*x\NodeType)\strtoken )
            Else
               EmitInt( *x\NodeType )
            EndIf
         ...
         Case #ljNEGATE
            ...
            ; NEW: Emit typed negate for floats
      ...
      ForEach llObjects()
         Select llObjects()\code
            ...
            Case #ljPRTI
               ...
               ; Look back to find what's being printed (previous FETCH/PUSH or arithmetic op)
               If PreviousElement(llObjects())
                  If llObjects()\code = #ljFetch Or llObjects()\code = #ljPush
                     ...
                     If n >= 0 And n < gnLastVariable
                        ...
                        EndIf
                     EndIf
                  ; *** FIX #5: PostProcessor - Detect float/string operation results ***
                  ; Check if previous operation is a float arithmetic operation
