...
;- Procedures for generating VM bytecode from AST
   ...
   ; NOTE: FindVariableSlotByOffset() is now defined in c2-inc-v17.pbi for include order
   ...
   Procedure            EmitInt( op.i, nVar.i = -1 )
      ...
      ; V1.034.X: Debug at very start
      If op = #ljFetch
         PrintN(">>> EmitInt START op=FETCH nVar=" + Str(nVar) + " name='' + gVarMeta(nVar)
ame + "'"'")
      EndIf
      ...
      Else
         ...
         Debug "V1.034.X: EmitInt op=" + Str(op) + " (#ljFetch=" + Str(#ljFetch) + ") nVar=" + Str(nVar) + " IsLocal=" + Str(IsLocalVar(nVar)) + " paramOffset=" + Str(gVarMeta(nVar)\paramOffset)
         If nVar >= 0 And IsLocalVar(nVar)
            ; This is a local variable - convert to local opcode and translate index
            Debug "V1.034.X: EmitInt LOCAL branch - converting to local opcode"
            Select op
               Case #ljFetch
                  llObjects()\code = #ljLFETCH
                  llObjects()\i = gVarMeta(nVar)\paramOffset
                  Debug "V1.034.X: EmitInt emitted LFETCH code=" + Str(llObjects()\code) + " i=" + Str(llObjects()\i)
               Case #ljFETCHS
   ...
   ; V1.034.0: Helper to check if a #ljCall node has a non-void return type
   ; Returns #True if the function call returns a value that would be left on stack
   ; This is used in SEQ handling to drop unused return values (e.g., listFirst() as statement)
   Procedure.i CallHasReturnValue(*callNode.stTree)
      Protected funcId.i

      ; V1.034.0: Additional pointer safety checks
      If Not *callNode
         ProcedureReturn #False
      EndIf

      ; Check for obviously invalid pointer (first memory page is typically unmapped)
      If *callNode < 4096
         ProcedureReturn #False
      EndIf

      If *callNode\NodeType <> #ljCall
         ProcedureReturn #False
      EndIf

      funcId = Val(*callNode\value)
      If funcId = 0
         ProcedureReturn #False
      EndIf

      ; Check if it's a type conversion (str(), strf()) - these always return string
      If funcId = #ljITOS Or funcId = #ljFTOS
         ProcedureReturn #True
      EndIf

      ; V1.034.0: Save map positions to avoid corrupting outer iterator contexts
      PushMapPosition(mapBuiltins())
      PushMapPosition(mapModules())

      Protected hasReturn.i = #False

      ; Check built-in functions
      ForEach mapBuiltins()
         If mapBuiltins()\opcode = funcId
            ; Built-in function found - check if it returns a value
            ; Return types: #C2FLAG_INT, #C2FLAG_FLOAT, #C2FLAG_STR, or 0 for void
            If mapBuiltins()\returnType <> 0
               hasReturn = #True
            EndIf
            Break
         EndIf
      ...
      ; If not found in builtins, check user-defined functions
      If Not hasReturn
         ForEach mapModules()
            If mapModules()\function = funcId
               ; User function - check return type
      ...
      ; V1.034.0: Restore map positions
      PopMapPosition(mapModules())
      PopMapPosition(mapBuiltins())

      ProcedureReturn hasReturn
   EndProcedure
   ...
   Procedure            CodeGenerator( *x.stTree, *link.stTree = 0 )
      ...
      ; V1.034.6: ForEach loop variables
      Protected         *forEachColl.stTree, *forEachBody.stTree
      Protected         foreachIsMap.i, foreachSlot.i, foreachVarName.s
      ; V1.034.0: For DROP on statement-level function calls
      Protected         seqCallFuncId.i
      ...
      Select *x\NodeType
         ...
         Case #ljPTRFIELD_I
            ; V1.034.X: Console debug for pointer issue
            PrintN(">>> PTRFIELD_I: left=" + Str(*x\left) + " value='" + *x\value + "'")
            If *x\left
               ; V1.034.X: Check if *x\left is an IDENT node (local variable pointer copy)
               ; If so, we need to emit LFETCH/FETCH for it, not call CodeGenerator
               PrintN(">>> PTRFIELD_I: left NodeType=" + Str(*x\left\NodeType) + " value='" + *x\left\value + "' IDENT=" + Str(#ljIDENT))
               If *x\left\NodeType = #ljIDENT
                  ; Simple variable (copied pointer like p = ptr; then p\i)
                  n = FetchVarOffset(*x\left\value)
                  PrintN(">>> PTRFIELD_I IDENT: n=" + Str(n) + " name='" + gVarMeta(n)\name + "' paramOffset=" + Str(gVarMeta(n)\paramOffset))
                  EmitInt( #ljFetch, n )    ; Fetch pointer variable
                  gVarMeta( n )\flags = gVarMeta( n )\flags | #C2FLAG_IDENT
               Else
                  ; Array element pointer field: arr[i]\i
                  PrintN(">>> PTRFIELD_I: calling CodeGenerator for left child")
                  CodeGenerator( *x\left )  ; Generate array access (leaves pointer on stack)
               EndIf
            Else
               ; Simple variable pointer field: ptr\i
               n = FetchVarOffset(*x\value)
               PrintN(">>> PTRFIELD_I LEAF: n=" + Str(n) + " name='" + gVarMeta(n)\name + "' paramOffset=" + Str(gVarMeta(n)\paramOffset))
               EmitInt( #ljFetch, n )    ; Fetch pointer variable
               gVarMeta( n )\flags = gVarMeta( n )\flags | #C2FLAG_IDENT
            EndIf
         ...
         Case #ljPTRFIELD_F
            If *x\left
               ; V1.034.X: Check if *x\left is an IDENT node (local variable pointer copy)
               If *x\left\NodeType = #ljIDENT
                  ; Simple variable (copied pointer like p = ptr; then p\f)
                  n = FetchVarOffset(*x\left\value)
                  EmitInt( #ljFetch, n )    ; Fetch pointer variable
                  gVarMeta( n )\flags = gVarMeta( n )\flags | #C2FLAG_IDENT
               Else
         ...
         Case #ljPTRFIELD_S
            If *x\left
               ; V1.034.X: Check if *x\left is an IDENT node (local variable pointer copy)
               If *x\left\NodeType = #ljIDENT
                  ; Simple variable (copied pointer like p = ptr; then p\s)
                  n = FetchVarOffset(*x\left\value)
                  EmitInt( #ljFetch, n )    ; Fetch pointer variable
                  gVarMeta( n )\flags = gVarMeta( n )\flags | #C2FLAG_IDENT
               Else
         ...
         Case #ljOr, #ljAND, #ljMOD, #ljXOR, #ljSHL, #ljSHR  ; V1.034.4: Added bit shift operators
            CodeGenerator( *x\left )
            CodeGenerator( *x\right )
            EmitInt( *x\NodeType)
