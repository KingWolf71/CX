...
$testFiles = Get-ChildItem ".\Examples\*.cx" | Where-Object { $_.Name -notmatch "bug|error|064|069|120|122" } | Sort-Object Name
$passed = 0
$failed = 0
$failedList = @()
...
foreach ($f in $testFiles) {
    $output = & .\cx.exe --test $f.FullName 2>&1 | Out-String
