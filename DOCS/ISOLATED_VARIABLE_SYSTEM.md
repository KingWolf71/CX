...
As of version 1.31.0, CX uses an **isolated variable system** where globals, locals, and the evaluation stack are stored in completely separate arrays. This eliminates the possibility of stack/local overlap bugs that could occur in the previous unified system (V1.18.0).
